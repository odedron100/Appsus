...
const BOOKS_SEARCH_KEY = 'booksSearch';
...
function getNewBooks(bookSearch) {
    let booksFound = utilService.loadFromStorage(BOOKS_SEARCH_KEY) || {};
    if (booksFound[bookSearch]) {
        console.log('booksFound localStrogae', booksFound);
        return Promise.resolve(booksFound[bookSearch]);
    }

    return fetch(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${bookSearch}`)
        .then(response => response.json())
        .then(data => {
            booksFound[bookSearch] = data;
            utilService.saveToStorage(BOOKS_SEARCH_KEY, booksFound);
